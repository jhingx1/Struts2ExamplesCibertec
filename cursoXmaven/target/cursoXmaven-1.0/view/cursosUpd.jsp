...
<html>
    ...
    <body>
        <s:form action="cursosUpd" theme="simple" method="post" 
                cssStyle="margin: auto;display: table" cssClass="parainfo">
            <s:hidden name="cursos.idcursos"/>
        ...
        <div class="msg_error errorMessage ui-state-highlight ui-corner-all">
            ...
            <ul> <%-- para errores por validacion --%>
                <s:if test="fieldErrors.get('cursos.idautor').size() > 0">
                    <li>
                        <s:property value="fieldErrors.get('cursos.idautor').get(0)" />
                    </li>
                </s:if>
                <s:if test="fieldErrors.get('cursos.cursos').size() > 0">
                    <li>
                        <s:property value="fieldErrors.get('cursos.cursos').get(0)" />
                    </li>
                </s:if>
            </ul>
        </div>
